/**
 * Arayüz metinleri. Sağlayıcıdan (i18n.tsx) ayrı duruyor çünkü derleme zamanı
 * çalışan ön-derleme adımı (vite.config.ts) da aynı metinleri okur; o adım
 * Node içinde koştuğu için React'e dokunamaz.
 */
...
export const STRINGS = {
  tr: {
    ...
    // Gönderim kanalı (notify_transports — components/MailTransport.tsx)
    mtTitle: 'Gönderim kanalı',
    mtSub: 'Bildirimleriniz varsayılan olarak FRAUDE sunucusundan gelir. İsterseniz kendi altyapınızdan çıkmalarını sağlayabilirsiniz.',
    mtKind: 'Kanal',
    mtKindPlatform: 'FRAUDE sunucusu (varsayılan)',
    mtKindTelegram: 'Telegram Botu (@FraudeTerminal_Bot)',
    mtKindWebhook: 'Kendi webhook ucum',
    mtKindApi: 'Kendi mail sağlayıcım',
    mtPlatformNote: 'Bildirimler FRAUDE’nin kendi mail altyapısından gönderilir. Ek kurulum gerekmez.',
    mtWebhookUrl: 'Webhook adresi (https)',
    mtWebhookSecret: 'İmzalama sırrı (isteğe bağlı)',
    mtWebhookNote: 'Her bildirim bu adrese JSON olarak POST edilir; maile hiç dönüşmez. Sır tanımlarsanız gövde X-Fraude-Signature başlığında HMAC-SHA256 ile imzalanır ve kaynağı doğrulayabilirsiniz.',
    mtProvider: 'Sağlayıcı',
    mtFromEmail: 'Gönderici adresi',
    mtFromName: 'Gönderici adı',
    mtApiKey: 'API anahtarı',
    mtApiNote: 'Gönderici adresinin alan adı sağlayıcınızda doğrulanmış olmalı, aksi hâlde test başarısız olur. Anahtar şifrelenerek saklanır ve bir daha gösterilmez.',
    mtSecretStored: '•••••••• kayıtlı — değiştirmek için yeni değer yazın',
    mtStatusActive: 'Kendi kanalınız etkin ve doğrulanmış.',
    mtStatusPlatform: 'Bildirimler şu an FRAUDE sunucusundan gönderiliyor.',
    mtDisabled: 'Kanalınız üst üste hata verdiği için kapatıldı. Bildirimleriniz kesilmedi, FRAUDE sunucusundan gitmeye devam ediyor. Ayarları düzeltip yeniden test edin.',
    mtLastError: 'Son hata',
    mtSave: 'Kaydet',
    mtSaveAndTest: 'Kaydet ve test et',
    mtSaving: 'Test ediliyor…',
    mtSavedPlatform: 'FRAUDE sunucusuna dönüldü. Kayıtlı anahtarınız silindi.',
    mtVerified: 'Doğrulandı ✓ — bundan sonra bildirimleriniz bu kanaldan gelecek.',
    mtFailed: 'Test başarısız: ',
    mtTooMany: 'Çok sık denediniz; birkaç saniye bekleyip tekrar deneyin.',
    mtSecurityNote: 'SMTP kullanıcı adı/şifresi istemiyoruz: SMTP şifresi çoğu sağlayıcıda posta kutunuzun tamamına gönderim yetkisi verir, API anahtarı ise yalnız gönderim yetkilidir ve tek tıkla iptal edilir.',
    ...
    // Bulunamayan adres
    nfTitle: 'Bu adreste bir şey yok',
    nfBody:
      'Bağlantı eski olabilir ya da yanlış yazılmış olabilir. Ana sayfadan devam edin veya modül künyesine göz atın.',
    nfHomeCta: 'Ana sayfa',
    nfModulesCta: 'Tüm modüller',
  ...
  en: {
    ...
    // Delivery transport (notify_transports — components/MailTransport.tsx)
    mtTitle: 'Delivery channel',
    mtSub: 'Your notifications are sent from FRAUDE’s own servers by default. If you prefer, they can go out through your own infrastructure instead.',
    mtKind: 'Channel',
    mtKindPlatform: 'FRAUDE servers (default)',
    mtKindTelegram: 'Telegram Bot (@FraudeTerminal_Bot)',
    mtKindWebhook: 'Custom webhook endpoint',
    mtKindApi: 'Custom mail provider',
    mtPlatformNote: 'Notifications are sent from FRAUDE’s own mail infrastructure. No setup required.',
    mtWebhookUrl: 'Webhook URL (https)',
    mtWebhookSecret: 'Signing secret (optional)',
    mtWebhookNote: 'Every notification is POSTed to this URL as JSON; it never becomes an email. If you set a secret, the body is signed with HMAC-SHA256 in the X-Fraude-Signature header so you can verify the origin.',
    mtProvider: 'Provider',
    mtFromEmail: 'Sender address',
    mtFromName: 'Sender name',
    mtApiKey: 'API key',
    mtApiNote: 'The sender address domain must be verified with your provider, otherwise the test will fail. The key is stored encrypted and never shown again.',
    mtSecretStored: '•••••••• stored — type a new value to replace it',
    mtStatusActive: 'Your own channel is active and verified.',
    mtStatusPlatform: 'Notifications are currently sent from FRAUDE servers.',
    mtDisabled: 'Your channel failed repeatedly and was switched off. Your notifications did not stop — they keep coming from FRAUDE servers. Fix the settings and test again.',
    mtLastError: 'Last error',
    mtSave: 'Save',
    mtSaveAndTest: 'Save and test',
    mtSaving: 'Testing…',
    mtSavedPlatform: 'Switched back to FRAUDE servers. Your stored key was deleted.',
    mtVerified: 'Verified ✓ — your notifications will now come through this channel.',
    mtFailed: 'Test failed: ',
    mtTooMany: 'Too many attempts; wait a few seconds and try again.',
    mtSecurityNote: 'We do not ask for SMTP credentials: an SMTP password grants send access to your whole mailbox with most providers, while an API key is send-only and can be revoked in one click.',
    ...
    nfTitle: 'Nothing lives at this address',
    nfBody:
      'The link may be out of date or mistyped. Continue from the home page, or browse the module list.',
    nfHomeCta: 'Home',
    nfModulesCta: 'All modules',
