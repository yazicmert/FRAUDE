...
function istanbulParts(now: Date): { weekday: number; minutes: number; hhmm: string } {
  ...
  return { weekday, minutes: hour * 60 + minute, hhmm };
}

export function getMarketStatus(now: Date = new Date()): MarketStatus {
  const { weekday, minutes, hhmm } = istanbulParts(now);
