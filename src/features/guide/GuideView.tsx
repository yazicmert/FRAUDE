...
const GUIDE: Record<'tr' | 'en', GuideContent> = {
  tr: {
    ...
    modules: [
      ...
      { code: 'FR', name: 'Forum', tag: 'Topluluk', desc: 'Kullanıcıların ortak alanı: konu aç, yanıtla, beğen. Metinde etiketlediğin hisse, konuyu o hissenin sayfasında da gösterir.', items: ['Gönderiler Supabase\'de ortaktır: birinin yazdığı konu diğer kullanıcıların uygulamasında da görünür', 'Etiketlemek için gövdeye $THYAO yazman yeter; "Hisse etiketle" kutusundan BIST evreninden de seçebilirsin', 'Etiketin karşılığı: aynı gönderi o hissenin sayfasındaki Forum bölümünde listelenir — hisse sayfasından yazınca etiket kendiliğinden eklenir', 'Tek seviye iş parçacığı: konu + yanıtlar, beğeni ve kendi gönderini silme', 'En çok konuşulanlar paneli son bir haftanın etiket sayımından; tıklayınca akış o hisseye iner', 'Yeni gönderiler canlı düşer; canlı yayın kapalıysa görünür sekmede dakikada bir yoklanır', 'Yazar adı sunucuda oturumdan yazılır, istemciden gelen ad yok sayılır; dakikada 8 gönderi sınırı vardır'] },
  ...
  en: {
    ...
    modules: [
      ...
      { code: 'FR', name: 'Forum', tag: 'Community', desc: 'The shared space for users: start a thread, reply, like. A ticker tagged in your text also shows the thread on that ticker\'s page.', items: ['Posts are shared in Supabase: a thread one user writes shows up in every other user\'s app', 'To tag, just type $THYAO in the body; you can also pick from the BIST universe via "Tag a ticker"', 'What the tag does: the same post is listed in the Forum section of that ticker\'s page — writing from a ticker page adds the tag automatically', 'Single-level threads: topic + replies, likes and deleting your own post', 'A most-discussed panel from the past week\'s tag counts; clicking one narrows the feed to that ticker', 'New posts arrive live; if realtime is off the visible tab polls once a minute', 'The author name is written server-side from the session, a client-supplied name is ignored; the rate limit is 8 posts per minute'] },
