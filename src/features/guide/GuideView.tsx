...
const GUIDE: Record<'tr' | 'en', GuideContent> = {
  tr: {
    ...
    modules: [
      ...
      { code: 'KP', name: 'Bilgi Deposu', tag: 'Bildirimler', desc: 'KAP bildirimleri, SPK bültenleri, aracı kurum analizleri ve haberler tek zaman çizgisinde.', items: ['Dört kaynak tek akışta; tür sekmeleriyle (KAP / SPK / Analiz / Haber) ayrılabiliyor', 'Tek arama kutusu hepsinde geçerli: başlık, hisse kodu, kaynak', 'Şirket bazlı özel durum açıklamaları, AI önem puanıyla', 'Kayıt açılınca kendi okuyucusunda: KAP bildirimi, SPK bülteni ve analiz raporu uygulamadan çıkmadan okunur', 'Bir kaynak düşerse depo boşalmaz, yalnız o bölüm eksik kalır'] },
      ...
      { code: 'CA', name: 'Kurumsal Olaylar', tag: 'Corporate', desc: 'Şirket aksiyonları ve halka arz takvimi.', items: ['Temettü kayıtları — KAP kar payı bildirimlerinden: pay grubu bazında brüt/net ve kesinleşen hak kullanım tarihi', 'Bedelli/bedelsiz sermaye artırımları — SPK bültenleri + KAP bildirimleri', 'Halka arz takvimi ve arşivi; fiyat, lot ve arz büyüklüğü SPK izahname onayından', 'Onay sonrası otomatik takip: katılımcı sayısı, ilk işlem tarihi, pazar ve endeks üyeliği KAP bildirimleri yayımlandıkça kendiliğinden dolar', 'Her satır kaynağını gösterir (SPK bülteni / KAP bildirimi)'] },
      { code: 'AR', name: 'Analiz Raporları', tag: 'Reports', desc: 'Aracı kurum ve bankaların araştırma raporları + analist konsensüsü. Hem kendi modülünde hem hisse sayfasında.', items: ['11 kurum akışı: İş Yatırım, Garanti BBVA Yatırım, Ziraat Yatırım, Gedik Yatırım, Ahlatcı Yatırım, PhillipCapital, Integral Yatırım, Vakıf Yatırım, Halk Yatırım, Marbaş Menkul, A1 Capital','Rapor başlığa tıklanınca KAP bildirimleri gibi uygulamanın içinde açılır: yakınlaştırma, sağ panele sabitleme, ilgili paya geçiş', 'Kurumların PDF\'leri çerçevelenmeye kapalı yayımlandığı için belge uygulama tarafından indirilip gömülür; açılamazsa tarayıcıya yönlendirir', 'Kurum / tür (şirket, sektör, strateji, bülten) filtreleri ve başlık-hisse-analist araması', 'Rapor başlığından tavsiye (AL/TUT/Endeks Üstü) ve hedef fiyat çıkarımı', 'Konsensüs sekmesi: payı izleyen kurum sayısı, AL/TUT/SAT dağılımı, ortalama hedef ve getiri potansiyeli', 'Yurt dışı bankaların BIST raporları aboneliğe kapalı; kapsamlarına konsensüs toplamlarından ulaşılır', 'Hisse açılınca o şirketin tam geçmişi üç kurumun hisse bazlı ucundan çekilir: İş Yatırım etiket akışı, Garanti arama, Ziraat hisse kategorisi'] },
  ...
  en: {
    ...
    modules: [
      ...
      { code: 'KP', name: 'Knowledge Base', tag: 'Disclosures', desc: 'KAP disclosures, SPK bulletins, broker research and news on one timeline.', items: ['Four sources in one stream, separable via type tabs (KAP / SPK / Research / News)', 'One search box across all of them: title, ticker, source', 'Company material disclosures with the AI importance score', 'Opening a record uses its own reader: KAP disclosures, SPK bulletins and research reports are read without leaving the app', 'If one source fails the base is not emptied — only that section is missing'] },
      ...
      { code: 'CA', name: 'Corporate Actions', tag: 'Corporate', desc: 'Corporate actions and IPO calendar.', items: ['Dividend records — from KAP dividend disclosures: gross/net per share class and the confirmed ex-date', 'Rights / bonus capital increases — SPK bulletins + KAP disclosures', 'IPO calendar and archive; price, lots and deal size from the SPK prospectus approval', 'Automatic follow-up after approval: investor count, first trading date, market and index membership fill themselves in as the KAP disclosures land', 'Every row shows its source (SPK bulletin / KAP disclosure)'] },
      { code: 'AR', name: 'Research Reports', tag: 'Reports', desc: 'Brokerage and bank research reports plus analyst consensus — in its own module and on the stock page.', items: ['11 broker feeds: İş Yatırım, Garanti BBVA Yatırım, Ziraat Yatırım, Gedik Yatırım, Ahlatcı Yatırım, PhillipCapital, Integral Yatırım, Vakıf Yatırım, Halk Yatırım, Marbaş Menkul, A1 Capital','Clicking a report title opens it inside the app like a KAP disclosure: zoom, dock to right panel, jump to the related ticker', 'Brokers publish their PDFs with framing disabled, so the document is downloaded by the app and embedded; if that fails it falls back to the browser', 'Broker / type (company, sector, strategy, bulletin) filters and title-ticker-analyst search', 'Rating (Buy/Hold/Outperform) and target price extracted from the report', 'Consensus tab: number of covering institutions, BUY/HOLD/SELL split, average target and upside', 'Foreign banks\' BIST reports are subscription-gated; their coverage is reached through consensus aggregates', 'Opening a stock pulls its full history from three per-ticker endpoints: İş Yatırım tag feed, Garanti keyword search, Ziraat per-ticker category'] },
