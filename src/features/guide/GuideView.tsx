...
const GUIDE: Record<'tr' | 'en', GuideContent> = {
  tr: {
    ...
    modules: [
      ...
      { code: 'KB', name: 'Bilgi Deposu', tag: 'Depo', desc: 'KAP bildirimleri, SPK bültenleri, 12 kurumun analiz raporları ve haberler tek zaman çizgisinde; istediğin şirkete odaklanıp o şirketin her kaydına tek ekrandan ulaşırsın.', items: ['Dört kaynak tek akışta; tür sekmeleriyle (KAP / SPK / Analiz / Haber / Konsensüs) ayrılabiliyor', 'SPK haftalık bültenleri üç yıllık dizinden gelir (~180 bülten), günde bir kez tazelenir', 'Kaynaklar birbirini beklemez: her biri geldiği anda çizilir, bekleyenler adıyla gösterilir', 'Derin tarama yalnız \"Yenile\" ile; açılış sığ tazeleme yapar ki ekran beklemesin', 'Şirket odağı: bir kod yazılınca dört kaynak da o şirkete indirgenir ve o payın rapor geçmişi VE KAP bildirim geçmişi kendi uçlarından yeniden çekilir (İş Yatırım etiket akışı, Garanti arama, Ziraat hisse kategorisi)', '12 kurumun analiz raporu: İş Yatırım, Garanti BBVA Yatırım, Ziraat Yatırım, Gedik Yatırım, Ahlatcı Yatırım, PhillipCapital, Integral Yatırım, Şeker Yatırım, Vakıf Yatırım, Halk Yatırım, Marbaş Menkul, A1 Capital', 'Kayıt açılınca kendi okuyucusunda: KAP bildirimi, SPK bülteni ve analiz raporu uygulamadan çıkmadan okunur — kurumların PDF\'leri çerçevelenmeye kapalı yayımlandığı için belge indirilip gömülür', 'Tek arama kutusu hepsinde geçerli: başlık, hisse kodu, kaynak, analist', 'Analiz sekmesinde kurum ve tür (şirket, sektör, strateji, bülten) çipleri', 'Rapor başlığından tavsiye (AL/TUT/Endeks Üstü) ve hedef fiyat çıkarımı', 'Konsensüs sekmesi: payı izleyen kurum sayısı, AL/TUT/SAT dağılımı, ortalama hedef ve getiri potansiyeli; yurt dışı bankaların BIST raporları aboneliğe kapalı olduğu için kapsamlarına yalnız bu toplamlardan ulaşılır', 'Şirket bazlı özel durum açıklamaları, AI önem puanıyla', 'Bir kaynak düşerse depo boşalmaz, yalnız o bölüm eksik kalır ve uyarı gösterilir'] },
  ...
  en: {
    ...
    modules: [
      ...
      { code: 'KB', name: 'Knowledge Base', tag: 'Base', desc: 'KAP disclosures, SPK bulletins, research from 12 brokers and news on one timeline; focus on any company and reach every record about it from a single screen.', items: ['Four sources in one stream, separable via type tabs (KAP / SPK / Research / News / Consensus)', 'SPK weekly bulletins come from a three-year index (~180 bulletins), refreshed once a day', 'Sources never block each other: each paints as it lands and pending ones are named', 'Deep scanning only on \"Refresh\"; startup does a shallow refresh so the screen never waits', 'Company focus: type a ticker and all four sources narrow to that company, while its report history is re-pulled from the brokers\' per-ticker endpoints (İş Yatırım tag feed, Garanti keyword search, Ziraat per-ticker category)', 'Research reports from 12 brokers: İş Yatırım, Garanti BBVA Yatırım, Ziraat Yatırım, Gedik Yatırım, Ahlatcı Yatırım, PhillipCapital, Integral Yatırım, Şeker Yatırım, Vakıf Yatırım, Halk Yatırım, Marbaş Menkul, A1 Capital', 'Opening a record uses its own reader: KAP disclosures, SPK bulletins and research reports are read without leaving the app — brokers publish their PDFs with framing disabled, so the document is downloaded and embedded', 'One search box across all of them: title, ticker, source, analyst', 'Broker and type (company, sector, strategy, bulletin) chips on the research tab', 'Rating (Buy/Hold/Outperform) and target price extracted from the report', 'Consensus tab: number of covering institutions, BUY/HOLD/SELL split, average target and upside; foreign banks\' BIST reports are subscription-gated, so their coverage is reached only through these aggregates', 'Company material disclosures with the AI importance score', 'If one source fails the base is not emptied — only that section is missing, and a warning is shown'] },
