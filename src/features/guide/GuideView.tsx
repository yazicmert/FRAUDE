...
const GUIDE: Record<'tr' | 'en', GuideContent> = {
  tr: {
    ...
    modules: [
      ...
      { code: 'CA', name: 'Kurumsal Olaylar', tag: 'Corporate', desc: 'Şirket aksiyonları ve halka arz takvimi.', items: ['Temettü kayıtları — KAP kar payı bildirimlerinden: pay grubu bazında brüt/net ve kesinleşen hak kullanım tarihi', 'Bedelli/bedelsiz sermaye artırımları — SPK bültenleri + KAP bildirimleri', 'Halka arz takvimi ve arşivi; fiyat, lot ve arz büyüklüğü SPK izahname onayından', 'Onay sonrası otomatik takip: katılımcı sayısı, ilk işlem tarihi, pazar ve endeks üyeliği KAP bildirimleri yayımlandıkça kendiliğinden dolar', 'Her satır kaynağını gösterir (SPK bülteni / KAP bildirimi)', 'Satış duyurusu (TSSD) okunur: arz fiyatı, talep toplama tarihleri ve satan ortakların adıyla pay yapısı — arz başlamadan önce, resmî belgeden', 'Payını satan ortağın tam unvanı SPK bülteninin dipnotlarından çıkarılır (kişi ya da şirket, ek satış dâhil)', 'Halka arz detay kartı ekran genişliğine oturur; tablo yatay kaydırılsa da kart yerinde kalır', 'Tahsisat oranı, fon kullanımı ve pay yapısı yalnız kaynakta yazdığı gibi gösterilir — okunamayan alan uydurulmaz, kaynak metni görünür'] },
  ...
  en: {
    ...
    modules: [
      ...
      { code: 'CA', name: 'Corporate Actions', tag: 'Corporate', desc: 'Corporate actions and IPO calendar.', items: ['Dividend records — from KAP dividend disclosures: gross/net per share class and the confirmed ex-date', 'Rights / bonus capital increases — SPK bulletins + KAP disclosures', 'IPO calendar and archive; price, lots and deal size from the SPK prospectus approval', 'Automatic follow-up after approval: investor count, first trading date, market and index membership fill themselves in as the KAP disclosures land', 'Every row shows its source (SPK bulletin / KAP disclosure)', 'The sale notice (TSSD) is parsed: offer price, book-building dates and the share structure with the selling shareholders\' names — from the official document, before the offering opens', 'The full legal name of each selling shareholder is extracted from the SPK bulletin footnotes (person or company, over-allotment included)', 'The IPO detail card fits the visible width and stays put while the table scrolls sideways', 'Allocation ratios, use of proceeds and share structure are shown only as the source states them — nothing unreadable is invented, the source text is displayed instead'] },
